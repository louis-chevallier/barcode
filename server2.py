...
class Reconstructor(threading.Thread):
    ...
    def __init__(self, server, args) :
        ...
        self.fitting = None
        if not args.dry :
            EKOX(args.geo_details)
            EKO(); self.fitting = iterative_fit.IterativeFit(args = args); EKO()
            EKOX( self.fitting.config.geo_details)             
        EKO()
        self.total=1
        self.running = False
    ...
    def process(self) -> bool :
        ...
        if "config" in req and NS(**req).config :
            EKOX(req)
            if NS(**req).config["active"] == '1' :
                EKO()
                ...
                if self.fitting is not None :
                    EKO()
                    self.fitting.release()
                    EKO(); self.fitting = None
                    iterative_fit.dogc()
                    gc.collect()                
                    self.queue.put({        
                        STATUS : "idle"
            ...
            def cb(x) :
                _,_,_,_,total = x
                self.total=total
                #EKON(total, len(self.progress))
                #EKOX(len(self.progress))
                self.progress.append("")
                #self.queue.put(x.progress)
            ...
            if self.fitting is None :
                ...
                for i in range(1,n+1) :
                    ...
                    time.sleep(1)
                    cb((1,2,3,4,n))
                with open("rparams.pckl", 'rb') as fd:
                    params = pickle.load(fd)
                    params[STATUS] = OK
                    params[REASON] = ""
            else :
                EKOX( self.fitting.config.geo_details)
                if 'run_photos' in req :
                    EKOT("start computation")
                    self.args.seq = dirphotos
                    self.last_objpath = no_path
                    try :
                        params = self.fitting.rundir(dirphotos, cb)
                        EKO()
                        params[STATUS] = OK
                        params[REASON] = ""
                    except Exception as e :
                ...
                if 'file' in req :
                    ...
                    self.args.seq = mp4
                    EKO()
                    params = self.fitting.processSeq(self.args, cb)
                EKOX(self.radic)
                try :
                    with open("rparams.pckl", 'wb') as fd:
                        pickle.dump(params, fd)
                except Exception as e:
                    EKOX(e)
                self.radic = params["radic"]
            x = _time.time()
            elapsed = (x - start)
            self.computationTime = "last computation time = %d s" % int(elapsed)
            EKOT("reconst done")
            ...
            if not os.path.exists("www/test/view_%05d.png" % vrdc) :
                EKOT("copy")
                for t in [ "view_%05d.png", "video_%05d.mp4", "radic_%05d.mtl", "radic_%05d.obj", "radic_%05d.png"] :
                    dd = t % vrdc
                    shutil.copyfile(os.path.join(self.args.dataroot, "results", t % ma99999),
                                    "www/test/%s" % dd)
            self.running = False                
            EKOX(vrdc)
            self.last_objpath =  params[OBJPATH]
            EKOX(list(params.keys()))
            d = {
                "computation_just_finished" : 1,
                OBJPATH : params[OBJPATH],
                "radic" : vrdc,
